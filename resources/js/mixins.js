    ...
    methods: {
        ...
        $onAjaxError(error, id) {
          ...
          if (error.response && error.response.status == 422 && error.response.data.field) {
            vm.openNotification(`Error ${error.response.status}: ${error.response.data.message}`, 'error');
            const validation = {};
            ...
            vm.openNotification(`Error ${error.response.status}: data not found`, 'error');
          }  else if(error.response && error.response.status == 401) {
            vm.openNotification(`Error ${error.response.status}: Access Denied`, 'error');
        ...
        alert(id, callback) {
          Modal.confirm({
            title: 'Konfirmasi',
            content: 'Apa anda yakin menghapus rekord ini?',
            onOk() {
              callback(id)
            },
            okText: 'Yes',
